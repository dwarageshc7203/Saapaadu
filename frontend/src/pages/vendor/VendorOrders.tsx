...
export default function VendorOrders() {
  ...
  const loadOrders = async () => {
    try {
      setLoading(true);
      const response = await api.get("/orders/vendor/my");
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-dark mb-2">Order Management</h1>
        <p className="text-gray-600">Manage and track all incoming orders from customers</p>
      </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {order.status === 'pending' && (
                      <div className="flex gap-2">
                        <button
                          ...
                          disabled={updatingOrder === order.oid}
                          className="btn btn-primary btn-sm"
                        >
                          {updatingOrder === order.oid ? "Updating..." : "Confirm"}
                        </button>
                        <button
                          onClick={() => updateOrderStatus(order.oid, 'cancelled')}
                          disabled={updatingOrder === order.oid}
                    ...
                    {order.status === 'confirmed' && (
                      <button
                        onClick={() => updateOrderStatus(order.oid, 'completed')}
                        disabled={updatingOrder === order.oid}
                        className="btn btn-success btn-sm"
                      >
                        {updatingOrder === order.oid ? "Updating..." : "Mark Complete"}
                      </button>
                    )}
                  </div>
                </div>
        ...
        <div className="card">
          <div className="card-body text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-600 mb-2">
              {statusFilter === "all" ? "No orders received yet" : `No ${statusFilter} orders`}
            </h3>
            <p className="text-gray-500">
              {statusFilter === "all" 
                ? "Create hotspots to start receiving orders from customers."
